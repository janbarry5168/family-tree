...
- `kinship.ts` — Dijkstra from the focused person under Taiwan civil-law 親等 counting (Civil Code §§ 968, 970). Spouse edges cost 0 (affinity mirrors the partner's blood degree), direct lineal (parent/child) edges cost 1 per generation, and collateral kin go up to the common ancestor and back — so siblings are degree 2 via the shared parent, not 1. Full siblings, half siblings, and older-generation siblings with incomplete parent data all fall out of the parent+child path naturally. When the shared parent isn't in the tree, the `Person.siblings` array contributes a cost-2 edge representing the implied me → parent → sibling path. Returns `Map<personId, degree>`. This degree is what drives the **degree filter** in the UI.
...
- `relationships.ts` — `getRelationshipLabel(focusedId, targetId, persons)` returns bilingual labels (`{en, zhTW}`) using a BFS path of edge tokens (`father|mother|spouse|child|sibling`) plus a gender/elder resolver. Extending kinship terms means adding a new `edges.length === N` case here — do NOT add a separate table elsewhere.

  **Kinship terminology — always verify against the source, never trust a summary.** When adding, auditing, or defending a label, open the source below and read; do not rely on memory or on prior code as authority. If a term isn't in any source, stop and ask.
  - 教育部《重編國語辭典修訂本》「親屬關係簡表」 — https://dict.revised.moe.edu.tw/appendix.jsp?ver=5&ID=15&ID2=8222
  - 中華民國民法 §§ 967–970 (親屬編，血親親等) — https://law.moj.gov.tw/LawClass/LawParaDeatil.aspx?pcode=B0000001&bp=103

  Coverage gap: 教育部辭典簡表 stops at first cousin (堂表兄弟姊妹); second-cousin-and-beyond has no official Taiwan naming. For same-generation kin beyond first cousin, the code collapses to 堂/表 + 兄/弟/姊/妹 by the patriline rule (同姓→堂, 不同姓→表) — see the generic same-gen far-kin case at the bottom of `getRelationshipLabel`.
